...
| ns | 役割 |
|----|------|
| `kagi.crypto` | ★ hybrid PQC エンベロープ（Provider seam: JVM=BouncyCastle / WASM=kotoba-crypto Rust） |
...
> ⚠️ 本リポは設計 ADR に対する **雛形(scaffold)**。`kagi.crypto` の PQC プリミティブは
> Provider プロトコル越しの参照実装で、BouncyCastle 配線と Rust/WASM 実装は段階導入。
